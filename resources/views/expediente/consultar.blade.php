<x-app-layout>
    ...
    <div class="expediente-container" x-data="{ modalNuevaCarpeta: false }">
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-5 mb-8">
            <h4 class="font-semibold text-gray-700 mb-3">Información del expediente</h4>
            <div class="space-y-2">
        ...
        <div class="flex items-center flex-wrap gap-1 text-sm mb-5">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            @forelse ($carpetas as $carpetaItem)
                <div class="relative border border-gray-200 rounded-lg p-4 hover:bg-gray-50 hover:border-gray-300 transition">
                    <a href="{{ route('expedientes.carpetas.index', [$expediente->id_expediente, $carpetaItem->id_carpeta]) }}"
                       class="flex items-center gap-2 text-gray-700 font-medium mb-3">
                        <span class="text-lg">📁</span>
                        <span class="truncate">{{ $carpetaItem->nombre }}</span>
                    </a>
                    <div class="relative z-10 flex gap-4 text-xs">
                        <a href="{{ route('expedientes.carpetas.editar', $carpetaItem->id_carpeta) }}"
                           class="relative z-10 text-[#550000] hover:underline py-1">
                            Editar
                        </a>
                        <form method="POST" action="{{ route('expedientes.carpetas.destroy', $carpetaItem->id_carpeta) }}"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="relative z-10 text-[#550000] hover:underline bg-transparent py-1 cursor-pointer">
                                Eliminar
                            </button>
                        </form>
                    </div>
        ...
        <div x-show="modalNuevaCarpeta" x-cloak
            ...
            <div class="bg-white rounded-lg p-6 w-full max-w-sm shadow-xl" @click.outside="modalNuevaCarpeta = false">
